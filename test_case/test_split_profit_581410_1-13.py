...
_LIST = list(map(int, re.findall(r"\d+\.?\d*", str(re.findall(r'"\d+"', BET_DETAIL)))))
print(_LIST)
BETS = len(re_results)
print('BETS', BETS)
OPEN_NUMBER = '99999'
...
class NacSplitProfit581410(object):
    def __init__(self):
    ...
    def get_split_number(self):
        result = self.split.split_581410(self.number, _LIST)
    ...
    def search_profit_loss(self):
        rec = _tool.get_profit_loss(SPLIT_LIST, '355') # random.choice(SPLIT_LIST)
    ...
    def check_split_detail(self):
        ...
        result = _tool.get_split_api_double_sided(lottery_code, bet_type_code, bet_detail)  # 获取API拆号结果
    ...
    def check_profit_loss(self, parameter):
        ...
        theory_bonus = self._sql.nac_theory_bonus('58_14_10_5')[1].get('theory_bonus')
...
if "__main__" == __name__:
    ...
    runner = Runner([NacSplitProfit581410], html_report=report + "\\nac_split_profit_report.html",
