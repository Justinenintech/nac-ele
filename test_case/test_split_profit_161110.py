...
print(AMOUNT)
SPLIT_DETAIL = _tool.bet_detail_duplex(BET_DETAIL, 3)
...
class NacSplitProfit161110(object):
    def __init__(self):
    ...
    def get_split_number(self):
        for parameter in SPLIT_DETAIL:
            ...
            # print('afters', afters)
            result = self.split.split_161110(self.number, int(before[0]), int(before[1]), int(before[2]), afters)
...
if "__main__" == __name__:
    ...
    runner = Runner([NacSplitProfit161110], html_report=report + "\\nac_split_profit_report.html",
