...
print(AMOUNT)
SPLIT_DETAIL = _tool.bet_detail_single(BET_DETAIL, 4)
...
class NacSplitProfit161211(object):
    def __init__(self):
    ...
    def get_split_number(self):
        for parameter in SPLIT_DETAIL:
            ...
            # print('afters', afters)
            result = self.split.split_161211(self.number, int(before[0]), int(before[1]), int(before[2]),
                                             int(before[3]), afters)
...
if "__main__" == __name__:
    ...
    runner = Runner([NacSplitProfit161211], html_report=report + "\\nac_split_profit_report.html",
