...
class NacSplitProfit181010(object):
    def __init__(self):
    ...
    def get_split_number(self):
        for parameter in SPLIT_DETAIL:
            ...
            # print('afters', afters)
            result = self.split.split_181010(self.number, int(before[0]), int(before[1]), afters)
    ...
    def search_profit_loss(self):
        rec = _tool.get_profit_loss(SPLIT_LIST, '88581')
    ...
    def check_profit_loss(self, parameter):
        ...
        print('count', count)
        if open_number[0] == open_number[1]:
            test_profit_loss = bet_amount * 1 - bet_amount
        else:
...
if "__main__" == __name__:
    ...
    runner = Runner([NacSplitProfit181010], html_report=report + "\\nac_split_profit_report.html",
