...
class NacSplitProfit161112(object):
    def __init__(self):
    ...
    def get_split_number(self):
        ...
        for parameter in SPLIT_DETAIL:
            ...
            result = self.split.split_161112(self.number, int(before[0]), int(before[1]), int(before[2]), int(afters))
            SPLIT_LIST.extend(result)

        [rlt.setdefault(i, ls.count(i)) for i in ls]
        [cs.append(int(self.split.get_bet_number_161112(v)) * int(list(rlt.values())[i])) for i, v in
         enumerate(rlt.keys())]
        print('cs', cs)
        BETS = sum(cs)
        print('BETS', BETS)
...
if "__main__" == __name__:
    ...
    runner = Runner([NacSplitProfit161112], html_report=report + "\\nac_split_profit_report.html",
