...
from setuptools._vendor.more_itertools import sliced
...
print('after', after)
SPLIT_DETAIL = after.split(',')
print('SPLIT_DETAIL', SPLIT_DETAIL)
BETS = len(_tool.bet_ssc_161213(SPLIT_DETAIL))
print('_tool.bet_11x5_701112(SPLIT_DETAIL)', _tool.bet_ssc_161213(SPLIT_DETAIL))
# for  i in _tool.bet_ssc_161215(SPLIT_DETAIL):
#     print(i.keys())
DETAIL = _tool.bet_detail_161213(BET_DETAIL, 4, _tool.bet_ssc_161213(SPLIT_DETAIL))
print("_tool.bet_detail_161215(BET_DETAIL,4,_tool.bet_ssc_161215(SPLIT_DETAIL))",
      _tool.bet_detail_161213(BET_DETAIL, 4, _tool.bet_ssc_161213(SPLIT_DETAIL)))
# for i in _tool.bet_detail_161213(BET_DETAIL, 4, _tool.bet_ssc_161213(SPLIT_DETAIL)):
#     print(list(parameter.keys())[0][0])
#     print(i.values())
#     print("二重号", list(list(parameter.values())[0].keys())[0])
#     print("单号", list(list(parameter.values())[0].values())[0])
print('BETS', BETS)
SPLIT_LIST = []
OPEN_NUMBER = '99999'
PROFIT_LIST = []
...
class NacSplitProfit161213(object):
    def __init__(self):
    ...
    def get_split_number(self):
        for parameter in DETAIL:
            result = self.split.split_161213(self.number, int(list(parameter.keys())[0][0]),
                                             int(list(parameter.keys())[0][1]), int(list(parameter.keys())[0][2]),
                                             int(list(parameter.keys())[0][3]),
                                             list(list(parameter.values())[0].keys())[0],
                                             list(list(parameter.values())[0].values())[0])
    ...
    def search_profit_loss(self):
        rec = _tool.get_profit_loss(SPLIT_LIST, random.choice(SPLIT_LIST))  # random.choice(SPLIT_LIST)
    ...
    def check_split_detail(self):
        ...
        result = _tool.get_split_api(lottery_code, BET_CODE, BET_DETAIL)  # 获取API拆号结果
...
if "__main__" == __name__:
    ...
    runner = Runner([NacSplitProfit161213], html_report=report + "\\nac_split_profit_report.html",
