...
print(AMOUNT)
SPLIT_DETAIL = _tool.bet_detail_single_twin(BET_DETAIL, 3,2)
print(SPLIT_DETAIL)
...
class NacSplitProfit161113(object):
    def __init__(self):
    ...
    def get_split_number(self):
        for parameter in SPLIT_DETAIL:
            ...
            result = self.split.split_161113(self.number, int(before[0]), int(before[1]), int(before[2]), str(afters[0]),str(afters[1]))
...
if "__main__" == __name__:
    ...
    runner = Runner([NacSplitProfit161113], html_report=report + "\\nac_split_profit_report.html",
