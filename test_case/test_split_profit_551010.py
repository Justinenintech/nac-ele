...
# print('_DETAIL',list(it.combinations(_DETAIL, 3)))
# SPLIT_DETAIL = [''.join(list(map(str, i))) for i in list(it.combinations(_DETAIL, 2))]
...
class NacSplitProfit551010(object):
    def __init__(self):
    ...
    def get_split_number(self):
        for parameter in SPLIT_DETAIL:
            result = self.split.split_551010(self.number, int(parameter[0]),int(parameter[1]))
...
if "__main__" == __name__:
    ...
    runner = Runner([NacSplitProfit551010], html_report=report + "\\nac_split_profit_report.html",
