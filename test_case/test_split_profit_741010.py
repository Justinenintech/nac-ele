...
print(AMOUNT)
SPLIT_DETAIL = _tool.bet_11x5_741010(BET_DETAIL)
ls = []
for i in SPLIT_DETAIL:
    ls.append(len(list(i.values())[0]))
BETS = sum(ls)
SPLIT_LIST = []
OPEN_NUMBER = '99999'
PROFIT_LIST = []
...
class NacSplitProfit741010(object):
    def __init__(self):
    ...
    def get_split_number(self):
        for parameter in SPLIT_DETAIL:
            result = self.split.split_741010(list(parameter.values())[0],list(parameter.keys())[0])
...
if "__main__" == __name__:
    ...
    runner = Runner([NacSplitProfit741010], html_report=report + "\\nac_split_profit_report.html",
