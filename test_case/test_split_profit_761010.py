...
# @Author  : Eagle
# @Site    : 
# @File    : test_split_profit_161113.py
# @Time    : 2021/6/28 0:59
# @Software: PyCharm
import random
import re
...
a = r'digitPos":"(.*?)",'
slotList1 = re.findall(a, BET_DETAIL)
print(slotList1)
c = r'numbers":"(.*?)",'
slotList2 = re.findall(c, BET_DETAIL)
print(slotList2)
print(list(zip(slotList1,slotList2)))
SPLIT_DETAIL = list(zip(slotList1,slotList2))
BETS = len(re_results)
print('BETS', BETS)
OPEN_NUMBER = '99999'
PROFIT_LIST = []
...
class NacSplitProfit761010(object):
    def __init__(self):
    ...
    def get_split_number(self):
        for parameter in SPLIT_DETAIL:
            result = self.split.split_761010(parameter[0], parameter[1])
            LISTS = [''.join(list(map(str, i))) for i in result]
            SPLIT_LIST.extend(LISTS)
        return SPLIT_LIST
    ...
    def res_be_detail(self):
        ...
        while True:
            try:
                ...
                res = _tool.get_double_sided_params(1,-15,lot_code=LOT_CODE, bet_code=BET_CODE,
                                                    amount=AMOUNT,
    ...
    def check_split_detail(self):
        ...
        result = _tool.get_split_api_double_sided(lottery_code, BET_CODE, BET_DETAIL)  # 获取API拆号结果
...
if "__main__" == __name__:
    ...
    runner = Runner([NacSplitProfit761010], html_report=report + "\\nac_split_profit_report.html",
