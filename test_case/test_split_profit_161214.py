...
print(AMOUNT)
SPLIT_DETAIL = _tool.bet_detail_single_twin(BET_DETAIL, 4, 2)
print(SPLIT_DETAIL)
...
class NacSplitProfit161214(object):
    def __init__(self):
    ...
    def get_split_number(self):
        for parameter in SPLIT_DETAIL:
            ...
            result = self.split.split_161214(self.number, int(before[0]), int(before[1]), int(before[2]),
                                             int(before[3]), str(afters[0]), str(afters[1]))
...
if "__main__" == __name__:
    ...
    runner = Runner([NacSplitProfit161214], html_report=report + "\\nac_split_profit_report.html",
