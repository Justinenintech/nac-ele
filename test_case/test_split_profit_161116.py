...
print(AMOUNT)
SPLIT_DETAIL = _tool.bet_detail_single_twin(BET_DETAIL, 3,3)
print(SPLIT_DETAIL)
...
class NacSplitProfit161116(object):
    def __init__(self):
    ...
    def get_split_number(self):
        for parameter in SPLIT_DETAIL:
            ...
            result = self.split.split_161116(self.number, int(before[0]), int(before[1]), int(before[2]), str(afters[0]),str(afters[1]),str(afters[2]))
...
if "__main__" == __name__:
    ...
    runner = Runner([NacSplitProfit161116], html_report=report + "\\nac_split_profit_report.html",
