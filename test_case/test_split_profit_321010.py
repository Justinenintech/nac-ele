...
print(AMOUNT)
# SPLIT_DETAIL = list(map(int, BET_DETAIL.split('|')))
...
class NacSplitProfit321010(object):
    def __init__(self):
        self._sql = MysqlPool('DATA_BASE')
        self.mysql = MysqlPool("SPLIT_BASE")
        self.split = Split()
        # self.number = list(it.combinations_with_replacement(list(itertools.islice(range(7), 1, 7)), 3))

    # @beforeClass()
    # def get_split_number(self):
    #     result = self.split.split_501010(self.number, SPLIT_DETAIL)
    #     _LIST = [''.join(list(map(str, i))) for i in result]
    #     SPLIT_LIST.extend(_LIST)
    #     return SPLIT_LIST
    #
    # # noinspection PyGlobalUndefined
    # @beforeClass(retry_on=[AssertionError, IndexError, ConnectionError])
    # def search_profit_loss(self):
    #     rec = _tool.get_profit_loss(SPLIT_LIST, '111') # random.choice(SPLIT_LIST)
    ...
    #             res = _tool.get_double_sided_params(20, -3, lot_code=LOT_CODE, bet_code=BET_CODE,
    #                                                 amount=AMOUNT,
    #                                                 betDetail=BET_DETAIL, bets=BETS,type=3)
    #             print('res', res)
    #             # print('res', type(res))
    ...
    def check_split_detail(self):
        ...
        issue_result = _tool.get_issue(LOT_CODE, 'dur', 20, -3)
        print('issue_result', issue_result)
        issue = str(int(issue_result) + 1)
        print('issue', issue)
        result = _tool.get_split_api_pk10_sided(LOT_CODE,BET_CODE , BET_DETAIL,issue)  # 获取API拆号结果
        ...
        # for i in result[1]:
        #     # lis = i.replace(',', ',')
        #     # bonus_result.append(lis)
        #     print('i',i)
        # print('bonus_result',bonus_result)
        #
        ...
        for i, v in enumerate(BET_DETAIL.replace('|', ' ').split(',')):
            if v != '#':
                ls.append({v: str(i)})
        print(ls)
        theory_bonus = self._sql.nac_theory_bonus(BET_CODE)[1].get('theory_bonus')
        print('theory_bonus',theory_bonus)
        _bonus = [-float(theory_bonus)]
        print('set_theory_bonus',set(_bonus))
        for parameter in ls:
            for i in result[1].items():
                # print(i[0],i[1])
                # print('list(map(int,i[0].split(',')))[int(''.join(parameter.values()))]',list(map(int,i[0].split(',')))[int(''.join(parameter.values()))])
                # print('ddd',list(map(int,''.join(parameter.keys()).split(' '))))
                if int(list(map(int,i[0].split(',')))[int(''.join(parameter.values()))]) in list(map(int,''.join(parameter.keys()).split(' '))):
                    # print(i[1])
                    bonus_result.append(i[1])
                    # print('bonus_resultbonus_resultbonus_result',bonus_result)
                else:
                    false_bonus_result.append(i[1])
                    # pass
        print('bonus_result',bonus_result)
        print('bonus_result_len',len(bonus_result))
        print('set',set(bonus_result))
        false_num = [0.0]
        print('false_bonus_result',false_bonus_result)
        print('set_false_bonus_result',set(false_bonus_result))
        print('false_num',set(false_num))
        assert_that(set(false_num)).is_equal_to(set(false_bonus_result))
        assert_that(set(_bonus)).is_equal_to(set(bonus_result))
        # for i in api_split_result:
        #     print(sum(list(map(int,i.split(',')))[:3]))
        #     if sum(list(map(int,i.split(',')))[:3]) in SPLIT_DETAIL:
    ...
    #     # 从数据库获取玩法对应的奖金
    #
    #     theory_bonus = self._sql.nac_theory_bonus('50_10_10_1')[1].get('theory_bonus')
    #
...
if "__main__" == __name__:
    ...
    runner = Runner([NacSplitProfit321010], html_report=report + "\\nac_split_profit_report.html",
